...
Use `npm run ai:prepare` after `setup:local` to run `ollama pull llama3.2` for the default local model and write `.tmp/local-ai-prepare/` evidence before doctor or strict local AI smoke. Final readiness and review-bundle verification require that prepared model to match the latest acceptance strict local AI model. It prepares only the local Ollama model; it does not create command payloads, bypass operator validation, validate hardware, or enable command upload.
