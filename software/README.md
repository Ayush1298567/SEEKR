...
Review bundles surface the fresh-clone local HEAD, clone HEAD, source-control local HEAD, source-control remote default SHA, source-control shallow fresh-clone HEAD, source-control npm dry-run result, source-control checked startup-file count, and plug-and-play setup generatedAt/status at top level, and `handoff:bundle:verify` compares those summaries with the copied source artifacts.
