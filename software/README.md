...
`npm run ai:prepare` runs `ollama pull llama3.2` for the default local model and writes `.tmp/local-ai-prepare/` evidence before doctor or strict local AI smoke. It prepares only the local Ollama model; it does not create command payloads, bypass operator validation, validate hardware, or enable command upload.
`npm run test:ai:local` runs strict Ollama smoke cases for normal assignment, prompt-injection notes, spatial metadata injection, and conflict-driven no-fly drafting. It requires Ollama and `llama3.2:latest` unless `SEEKR_OLLAMA_MODEL` points to another local model, then writes `.tmp/ai-smoke-status.json` for `/api/readiness`; acceptance rejects the marker unless it records a loopback Ollama URL, those named safety scenarios are present, and each saved case records the selected plan kind, validator pass, no unsafe operator-facing text, and no mutation while thinking. Spatial asset, passive-plan, incident-log, readiness, and operator-input read tools remain advisory and are covered by Vitest/API/UI checks.
...
Use `npm run handoff:bundle -- --label <review-name>` after handoff verification, `health:gstack`, `audit:gstack`, `audit:source-control`, `audit:todo`, `setup:local`, `ai:prepare`, `smoke:rehearsal:start`, and a final standalone `doctor` rerun to copy the verified handoff index, linked local artifacts, latest gstack workflow-status artifact, latest referenced gstack browser QA report and screenshots, latest TODO audit artifact, latest source-control handoff artifact, latest plug-and-play setup artifact, latest local AI prepare artifact, latest operator-start plug-and-play doctor artifact, latest rehearsal-start smoke artifact, strict local AI smoke status, and `docs/OPERATOR_QUICKSTART.md` into `.tmp/handoff-bundles/` for internal review. The bundle command verifies the handoff digest table and requires passing or limitation-only workflow status with `gstackAvailable: true`, exact ordered workflow and perspective rows, `pass-with-limitations` for limitation-only evidence, preserved manifest-level limitation details including unavailable gstack CLI details, preserved helper-tool root/count/name evidence when the umbrella CLI is unavailable, preserved limitation details for stale or missing health/QA evidence, per-workflow skill availability, Git review evidence paths when Git metadata is present or no-Git limitations when the workspace lacks `.git`, health-history status/path, QA screenshot path preservation, TODO artifacts with exact ordered blocker category IDs, matching top-level/category blocker counts, well-formed supporting TODO/blocker match evidence, a read-only source-control handoff artifact with GitHub/local-Git publication, local-head, remote default branch SHA, `workingTreeClean`, and status-line count state surfaced at the bundle-manifest top level for review, no ready source-control handoff generated before the latest acceptance record, passing plug-and-play setup evidence, passing local AI prepare evidence from `npm run ai:prepare`, passing operator-start plug-and-play doctor evidence for repository safety, matching source-control handoff path preservation, runtime dependencies, startup checks, and freshness against acceptance, exact ordered rehearsal-start smoke evidence for startup/source-health/readiness/shutdown, strict local AI smoke proof that matches acceptance and preserves a loopback Ollama URL plus per-case plan-kind, validator, unsafe-text, and mutation safety fields, and operator quickstart coverage for GitHub clone plus `cd SEEKR/software`, setup/ai:prepare/source-control audit/start, strict AI smoke proof, advisory-only Ollama AI that cannot create command payloads or bypass validation, API evidence, source-health, real-world blockers, and non-negated, non-contradictory disabled command/hardware authority first, keeps all authorization fields false, and does not validate hardware by itself.
Use `npm run handoff:bundle:verify` to verify the latest copied review bundle after packaging or transfer. Pass `-- --bundle .tmp/handoff-bundles/<file>.json` to verify a specific bundle manifest; it checks copied-file byte counts, SHA-256 digests, copied acceptance status with the exact required strict local AI scenario names and loopback Ollama URL, copied strict local AI smoke status with exact Ollama scenario order, acceptance-matched timestamp/model/loopback URL, per-case plan-kind, validator pass, no unsafe operator-facing text, and no thinking-time mutation, copied API-probe readback of that acceptance timestamp, command count, scenario names, loopback Ollama URL, release checksum, command-boundary scan, and disabled command upload, copied gstack workflow semantics including helper-tool evidence when the copied umbrella CLI is unavailable, exact workflow/perspective row order, and perspective status/score/next-action details, copied gstack QA report and screenshot inclusion plus report-content pass/command-safety, copied TODO audit exact blocker-category order/top-level-count/support-shape semantics, copied source-control handoff semantics including ready-artifact freshness against copied acceptance plus bundle-manifest repository URL/configured-remotes/local/remote SHA and clean-worktree summary agreement with the copied source-control artifact, copied plug-and-play setup exact env/data/safety row semantics, copied local AI prepare model-prep semantics, copied operator-start plug-and-play doctor exact repository-safety/source-control-handoff/runtime/start-wrapper/AI/port/data/safety row semantics including a source-control evidence path that matches the copied source-control handoff, freshness against copied acceptance, copied rehearsal-start smoke exact ordered startup/source-health/readiness/shutdown row semantics, copied operator quickstart GitHub clone/software-directory setup/ai:prepare/source-control audit/start/strict-AI-smoke/advisory-AI/API/source-health/safety guidance including no AI-created command payloads or validation bypass, requires critical doctor checks to pass while allowing only source-control/port/data-directory warnings, and full high-confidence secret-scan coverage while keeping hardware claims false.
Use `npm run audit:plug-and-play` after acceptance, API probe, `qa:gstack`, `health:gstack`, `audit:gstack`, `audit:source-control`, `audit:todo`, `setup:local`, `ai:prepare`, `smoke:rehearsal:start`, the final standalone `doctor`, and bundle verification to write `.tmp/plug-and-play-readiness/` JSON/Markdown evidence that the local app, local Ollama AI, API readback, QA report, review bundle, setup artifact, local AI prepare artifact, operator-start doctor artifact, rehearsal-start smoke artifact, operator quickstart, and safety boundary are ready for an internal-alpha plug-and-play operator while preserving real-world blockers. It directly matches the API probe's session-visible timestamp, local-AI summary including exact required strict smoke scenario names and loopback Ollama URL, release checksum, and command-boundary scan summaries to the latest acceptance status before allowing local plug-and-play readiness. It requires the current gstack workflow artifact to record installed gstack skill/tool availability, explicit `gstack` CLI availability, and helper-tool root/count/name evidence when helper tools are installed without the umbrella CLI. It also publishes top-level source-control and review-bundle summaries with repository URL, configured remotes, local HEAD, GitHub default-branch SHA, clean-worktree, status-line count, checked-file, and secret-scan state; checks that the verified review bundle points at the latest handoff bundle, gstack workflow status, gstack QA report, TODO audit, source-control handoff, plug-and-play setup, local AI prepare artifact, operator-start plug-and-play doctor, rehearsal-start smoke, strict local AI smoke status, and packaged operator quickstart; and fails if the bundle verification's source-control SHA or clean-worktree summary drifts from the latest source-control handoff. It also checks that the latest ready source-control handoff artifact passes the shared read-only semantic guard and was generated no earlier than the latest acceptance record before publication gaps are treated as warnings, that the operator-start doctor preflight is current relative to the latest acceptance record and references the latest source-control handoff artifact, that `docs/OPERATOR_QUICKSTART.md` covers GitHub clone plus `cd SEEKR/software`, setup/ai:prepare/source-control audit/start, strict local AI smoke proof, local AI as advisory-only with no command-payload creation or validation bypass, API evidence, source health, real-world blockers, and non-negated, non-contradictory disabled command/hardware authority, and that the setup/local-AI-prepare/operator-doctor/smoke artifacts contain the expected env/data/model-prep/package-script/runtime/repository-safety/source-control-handoff/start-wrapper/local-AI/port/safety/startup/readiness evidence with critical checks passing and only source-control/port/data-directory warnings allowed. It fails closed if the completion audit's `complete` flag and real-world blocker list disagree, so an empty malformed blocker list cannot clear aircraft/hardware readiness unless the completion audit explicitly reports `complete: true`.
Use `npm run audit:goal` after handoff verification, `audit:gstack`, `audit:todo`, bundling, bundle verification, and `audit:plug-and-play` to write `.tmp/goal-audit/` JSON/Markdown evidence that restates the objective as a prompt-to-artifact checklist. It verifies named docs including `docs/OPERATOR_QUICKSTART.md`, package scripts, safety-boundary evidence, acceptance/release/API probe evidence with session-visible timestamp, local-AI summary including strict smoke scenario names and loopback Ollama URL, release checksum, and command-boundary readback matching acceptance, completion audit, demo/handoff/bundle/gstack chain including local AI prepare and strict local AI smoke status pointers, plug-and-play readiness including current acceptance/API probe, source-control handoff, local AI prepare, operator quickstart references, warning details, top-level source-control repository URL/configured-remotes/SHA/clean-worktree summary agreement, review-bundle repository URL/configured-remotes/SHA/clean-worktree summary agreement, and persisted blocker-count/list agreement, TODO/blocker consistency, and its own persisted remaining-blocker count/list without claiming hardware validation.
