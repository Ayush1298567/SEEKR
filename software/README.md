...
Use `npm run audit:plug-and-play` after acceptance, API probe, `qa:gstack`, `health:gstack`, `audit:gstack`, `audit:source-control`, `audit:todo`, `setup:local`, `smoke:rehearsal:start`, the final standalone `doctor`, and bundle verification to write `.tmp/plug-and-play-readiness/` JSON/Markdown evidence that the local app, local Ollama AI, API readback, QA report, review bundle, setup artifact, operator-start doctor artifact, rehearsal-start smoke artifact, operator quickstart, and safety boundary are ready for an internal-alpha plug-and-play operator while preserving real-world blockers. It directly matches the API probe's session-visible timestamp, local-AI summary including exact required strict smoke scenario names and loopback Ollama URL, release checksum, and command-boundary scan summaries to the latest acceptance status before allowing local plug-and-play readiness. It requires the current gstack workflow artifact to record installed gstack skill/tool availability, explicit `gstack` CLI availability, and helper-tool root/count/name evidence when helper tools are installed without the umbrella CLI. It also checks that the verified review bundle points at the latest handoff bundle, gstack workflow status, gstack QA report, TODO audit, source-control handoff, plug-and-play setup, operator-start plug-and-play doctor, rehearsal-start smoke, strict local AI smoke status, and packaged operator quickstart, that the latest ready source-control handoff artifact passes the shared read-only semantic guard and was generated no earlier than the latest acceptance record before publication gaps are treated as warnings, that the operator-start doctor preflight is current relative to the latest acceptance record and references the latest source-control handoff artifact, that `docs/OPERATOR_QUICKSTART.md` covers GitHub clone plus `cd SEEKR/software`, setup/source-control audit/start, strict local AI smoke proof, local AI as advisory-only with no command-payload creation or validation bypass, API evidence, source health, real-world blockers, and non-negated, non-contradictory disabled command/hardware authority, and that the setup/operator-doctor/smoke artifacts contain the exact expected env/data/package-script/runtime/repository-safety/source-control-handoff/start-wrapper/local-AI/port/safety/startup/readiness rows in order with critical checks passing and only source-control/port/data-directory warnings allowed.
