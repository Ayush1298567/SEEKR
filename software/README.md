...
Use `npm run audit:source-control` to write `.tmp/source-control-handoff/` JSON/Markdown evidence for the GitHub handoff state. It checks package/README repository references, local Git metadata, configured remotes, GitHub remote refs/default branch with `git ls-remote`, whether local HEAD matches the GitHub default branch SHA, and whether the local worktree is clean. It persists an exact ordered source-control check list, blocked/warning check counts, plus a manual publication next-action checklist for missing `.git`, origin, refs, unpublished HEAD, dirty worktrees, and re-audit steps, but never initializes Git, commits, pushes, changes GitHub settings, validates hardware, or enables command upload.
