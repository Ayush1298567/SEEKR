...
`npm run ai:prepare` runs the exact `ollama pull llama3.2` command for the default local model and writes `.tmp/local-ai-prepare/` evidence before doctor or strict local AI smoke. Downstream validators require the prepare artifact to preserve the exact three-token `ollama pull <model>` shape and matching command evidence. It prepares only the local Ollama model; it does not create command payloads, bypass operator validation, validate hardware, or enable command upload.
...
Use `npm run ai:prepare` after `setup:local` to run `ollama pull llama3.2` for the default local model and write `.tmp/local-ai-prepare/` evidence before doctor or strict local AI smoke. Final readiness and review-bundle verification require that prepared model to match the latest acceptance strict local AI model and require the prepare command to be exactly three tokens, `ollama pull <model>` or an absolute `ollama` executable path plus `pull <model>`, with artifact check evidence containing the exact command string. It prepares only the local Ollama model; it does not create command payloads, bypass operator validation, validate hardware, or enable command upload.
