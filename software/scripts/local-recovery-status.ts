...
export function localRecoveryStatusCliSummary(result: { manifest: LocalRecoveryStatusManifest; jsonPath: string; markdownPath: string }) {
  return {
    ...
    strictAi: result.manifest.strictAi,
...
function renderMarkdown(manifest: LocalRecoveryStatusManifest) {
  return [
    ...
    manifest.strictAi?.provider ? `Strict AI provider: ${manifest.strictAi.provider}` : undefined,
    manifest.strictAi?.model ? `Strict AI model: ${manifest.strictAi.model}` : undefined,
    manifest.strictAi?.ollamaUrl ? `Strict AI URL: ${manifest.strictAi.ollamaUrl}` : undefined,
    manifest.strictAi?.caseCount !== undefined ? `Strict AI case count: ${manifest.strictAi.caseCount}` : undefined,
    ...
    manifest.reviewBundle?.status ? `Review bundle verification: ${manifest.reviewBundle.status}` : undefined,
    manifest.reviewBundle?.checkedFileCount !== undefined ? `Review bundle checked files: ${manifest.reviewBundle.checkedFileCount}` : undefined,
    manifest.reviewBundle?.secretScanStatus ? `Review bundle secret scan: ${manifest.reviewBundle.secretScanStatus}` : undefined,
    manifest.reviewBundle?.secretFindingCount !== undefined ? `Review bundle secret findings: ${manifest.reviewBundle.secretFindingCount}` : undefined,
