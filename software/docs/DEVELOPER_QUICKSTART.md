...
`npm run audit:source-control` writes `.tmp/source-control-handoff/` JSON/Markdown for the GitHub handoff state. It records package/README repository references, the parent GitHub landing README fresh-clone path into `SEEKR/software`, local Git metadata, configured remotes, the GitHub remote refs/default branch with `git ls-remote`, shallow fresh-clone contents for the landing README, `software/package.json`, and operator quickstart, local HEAD versus the GitHub default branch SHA, clean-worktree status, an exact ordered source-control check list, persisted blocked/warning check counts, and a manual publication next-action checklist for missing GitHub landing guidance, incomplete fresh-clone contents, `.git`, origin, refs, unpublished HEAD, dirty worktrees, and re-audit steps; it is read-only and never initializes Git, commits, pushes, changes GitHub settings, validates hardware, or enables command upload.
