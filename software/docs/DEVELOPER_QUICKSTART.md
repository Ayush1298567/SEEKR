...
`npm run handoff:index -- --label <handoff-name>` writes `.tmp/handoff-index/` JSON/Markdown after the demo package and bench packet. It verifies those artifacts point at the latest acceptance, release, completion audit, safety scan, API probe, hardware archive, policy gate, and overnight status before internal review, checks that acceptance and the demo package embed the same latest command-boundary scan and API-probe evidence, verifies API-probe readback for acceptance timestamp, command count, exact strict local AI scenario names, loopback Ollama URL, release checksum, and command-boundary scan, records SHA-256 digests for linked artifacts, and does not clear any real-world blocker.
...
`npm run audit:plug-and-play` writes `.tmp/plug-and-play-readiness/` JSON/Markdown after acceptance, API probe, gstack QA/health/workflow evidence, source-control handoff evidence, TODO audit, setup, local AI prepare, rehearsal-start smoke, the final operator-start doctor rerun, and bundle verification. It proves the local app, local Ollama AI smoke, API readback, browser QA, review bundle, setup artifact, local AI prepare artifact, operator-start doctor artifact, rehearsal-start smoke artifact, operator quickstart, rehearsal start wrapper, and safety boundary are ready for an internal-alpha plug-and-play operator while keeping physical hardware blockers explicit. API readback must match the latest acceptance timestamp, local-AI summary including exact required strict smoke scenario names and loopback Ollama URL, release checksum, and command-boundary scan summaries before the readiness audit can pass. The workflow/QA check now also requires current gstack workflow evidence to record installed gstack skill/tool availability, explicit `gstack` CLI availability, and helper-tool root/count/name evidence when helper tools are installed without the umbrella CLI. The manifest now publishes top-level source-control and review-bundle summaries with repository URL, configured remotes, local branch, remote default branch, remote ref count, blocked/warning check counts, local HEAD, GitHub default-branch SHA, clean-worktree, status-line count, checked-file, and secret-scan state, and it fails if the verified bundle's source-control branch/ref/count/SHA or clean-worktree summary drifts from the latest source-control handoff. Source-control handoff gaps are reported as warnings only after the ready artifact passes the shared read-only semantic guard and was generated no earlier than the latest acceptance record; unsafe, malformed, or pre-acceptance ready source-control handoff artifacts fail. It fails if the completion audit's `complete` flag, `summary.blocked`, blocked item IDs/details, real-world blocker ID list, or real-world blocker list disagree, the operator-start doctor predates acceptance, does not reference the latest source-control handoff artifact, has extra/reordered/missing package-script/runtime/repository-safety/source-control-handoff/start-wrapper/env/local-AI/port/data/safety rows, has a non-passing critical operator-doctor check, `docs/OPERATOR_QUICKSTART.md` omits GitHub clone plus `cd SEEKR/software`, setup/ai:prepare/source-control audit/start, occupied-port recovery, strict local AI smoke proof, local AI as advisory-only with no command-payload creation or validation bypass, API evidence, source-health, real-world blockers, or non-negated, non-contradictory disabled command/hardware authority, the operator start wrapper drops its local defaults or setup-before-ai-prepare-before-source-control-before-doctor-before-dev ordering, the rehearsal-start smoke artifact is missing, failing, extra-rowed, or reordered, or the verified review bundle points at stale handoff, gstack workflow, gstack QA, TODO audit, source-control handoff, plug-and-play setup, local AI prepare, operator-start plug-and-play doctor, rehearsal-start smoke, strict local AI smoke status, or packaged operator quickstart evidence.

`npm run handoff:bundle -- --label <review-name>` verifies the handoff digest table and requires passing or limitation-only gstack workflow-status with `gstackAvailable: true`, exact ordered health/review/planning/QA workflow rows, exact ordered operator/safety/DX/replay/demo-readiness perspective rows, `pass-with-limitations` for limitation-only evidence, preserved manifest-level limitation details including unavailable gstack CLI details, preserved helper-tool root/count/name evidence when the umbrella CLI is unavailable, preserved limitation details for stale or missing health/QA evidence, per-workflow skill availability, Git review evidence paths when Git metadata is present or no-Git limitations when the workspace lacks `.git`, health-history status/path from `health:gstack`, QA screenshot path preservation, TODO audit artifacts with exact ordered blocker category IDs, matching top-level/category blocker counts, well-formed supporting TODO/blocker match evidence, a read-only source-control handoff artifact with local Git/GitHub publication, local branch, local-head, remote default SHA, blocked/warning check counts, `workingTreeClean`, and status-line count state surfaced at the bundle-manifest top level for review, no ready source-control handoff generated before the latest acceptance record, a passing plug-and-play setup artifact, a passing local AI prepare artifact proving `ollama pull llama3.2`, a passing operator-start plug-and-play doctor artifact with repository-safety/source-control-handoff/runtime/startup evidence that is fresh against acceptance and matches the source-control handoff being packaged, a passing exact-row rehearsal-start smoke artifact that also records setup/AI/source-control/smoke-doctor artifact pointers, strict local AI smoke proof that matches acceptance and preserves a loopback Ollama URL plus per-case plan-kind, validator, unsafe-text, and mutation safety fields, and operator quickstart coverage for GitHub clone plus `cd SEEKR/software`, setup/ai:prepare/source-control audit/start, occupied-port recovery, strict AI smoke proof, local AI as advisory-only with no command-payload creation or validation bypass, API evidence, source-health, real-world blockers, and non-negated disabled authority, then copies the handoff index, linked local artifacts, the workflow artifact, the workflow-referenced local gstack browser QA report and screenshots, the TODO audit artifact, the source-control handoff artifact, the plug-and-play setup artifact, the local AI prepare artifact, the operator-start plug-and-play doctor artifact, the rehearsal-start smoke artifact, `.tmp/ai-smoke-status.json`, and `docs/OPERATOR_QUICKSTART.md` into `.tmp/handoff-bundles/` with a manifest for internal review. It is a packaging step only: all hardware claims and authorization fields remain false.

`npm run handoff:bundle:verify` verifies the latest copied bundle manifest, every copied file digest, copied acceptance status with the exact required strict local AI scenario names and loopback Ollama URL, copied strict local AI smoke status with exact Ollama scenario order, acceptance-matched timestamp/model/loopback URL, per-case plan-kind, validator pass, no unsafe operator-facing text, and no thinking-time mutation, copied API-probe readback of the same acceptance timestamp, command count, scenario names, loopback Ollama URL, release checksum, command-boundary scan, and disabled command upload, copied gstack workflow semantics including helper-tool evidence when the copied umbrella CLI is unavailable, exact workflow/perspective order, and perspective status/score/next-action detail, copied gstack QA report and screenshot inclusion plus report-content pass/command-safety, copied TODO audit exact blocker-category order/top-level-count/support-shape semantics, copied source-control handoff semantics including ready-artifact freshness against copied acceptance plus bundle-manifest repository URL/configured-remotes/local-branch/default-branch/ref-count/blocked-check-count/warning-check-count/local/remote SHA and clean-worktree summary agreement with the copied source-control artifact, copied plug-and-play setup exact env/data/safety row semantics, copied local AI prepare model-prep semantics including copied acceptance-model matching, copied operator-start plug-and-play doctor exact repository-safety/source-control-handoff/runtime/start-wrapper/AI/port/data/safety row semantics and freshness against copied acceptance, copied rehearsal-start smoke exact ordered setup/AI/source-control/smoke-doctor/startup/source-health/readiness/shutdown row semantics plus prerequisite artifact pointers, copied operator quickstart GitHub clone/software-directory setup/ai:prepare/source-control audit/start/occupied-port recovery/strict-AI-smoke/advisory-AI/API/source-health/safety guidance including no AI-created command payloads or validation bypass, critical operator-doctor check pass status with only source-control/port/data-directory warnings allowed, and full high-confidence secret-scan coverage. Use `npm run handoff:bundle:verify -- --bundle .tmp/handoff-bundles/<file>.json` for a specific review bundle after transfer.

`npm run audit:goal` writes `.tmp/goal-audit/` JSON/Markdown after handoff verification, `audit:gstack`, `audit:todo`, bundling, bundle verification, and `audit:plug-and-play`. It restates the objective as a prompt-to-artifact checklist across named files including `docs/OPERATOR_QUICKSTART.md`, commands, safety boundary, acceptance/release/API probe evidence that directly matches session-visible timestamp, local-AI summary including exact required strict smoke scenario names and loopback Ollama URL, release checksum, and command-boundary summaries to acceptance, completion audit, demo/handoff/bundle/gstack chain including local AI prepare and strict local AI smoke status pointers, plug-and-play readiness that is current against acceptance and references the current API probe, source-control handoff, local AI prepare, and operator quickstart evidence, plus plug-and-play readiness top-level source-control and review-bundle repository URL/configured-remotes/local-branch/default-branch/ref-count/blocked-check-count/warning-check-count/SHA/clean-worktree summaries that must match the latest handoff and bundle verification, plug-and-play blocker-count/ID/list agreement with the current completion audit, TODO/blocker consistency, `docs/goal.md`, and explicit real-world blockers with a persisted remaining-blocker count/ID/list.
