...
Review bundles publish the fresh-clone local HEAD, clone HEAD, source-control handoff HEADs, source-control shallow fresh-clone HEAD, source-control npm dry-run status, and source-control checked startup-file count at top level; `npm run handoff:bundle:verify` compares those fields with the copied source artifacts so reviewers can see clone agreement without opening nested JSON first.
