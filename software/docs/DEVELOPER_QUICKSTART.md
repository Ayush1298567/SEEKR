...
`npm run demo:package -- --label <demo-name>` writes `.tmp/demo-readiness/` JSON/Markdown after acceptance and completion audit. It packages the latest acceptance, release checksum, command-boundary scan, API probe, completion audit, hardware archive, policy gate, and overnight status pointers for an internal demo handoff while keeping all hardware claims false, listing real-world blockers from the audit, adding an operator/safety/DX/replay/demo-readiness perspective review, and adding a next-evidence checklist with the runbook and command that should produce each missing artifact. Stale overnight status is a warning, not a hardware validation claim.
...
`npm run handoff:index -- --label <handoff-name>` writes `.tmp/handoff-index/` JSON/Markdown after the demo package and bench packet. It verifies those artifacts point at the latest acceptance, release, completion audit, safety scan, API probe, hardware archive, policy gate, and overnight status before internal review, checks that acceptance and the demo package embed the same latest command-boundary scan and API-probe evidence, records SHA-256 digests for linked artifacts, and does not clear any real-world blocker.
...
`npm run audit:gstack` writes `.tmp/gstack-workflow-status/` JSON/Markdown with health, review, planning, QA, perspective status/score/next-action detail, per-workflow installed skill availability, whether the `gstack` CLI is on PATH, latest gstack health-history status/path, and the latest local `.gstack/qa-reports/` browser QA report status/path plus named screenshot paths when present. Run it before final bundling so the review bundle can copy the same workflow artifact that `audit:goal` will check. Health history that predates acceptance or lacks a parseable `ts` or `timestamp` field, stale browser QA, and no-Git review limitations remain explicit limitations; browser QA reports with failed check rows or missing referenced screenshots fail the workflow audit. Current local health and browser coverage still come from the latest acceptance/check/Playwright outputs.

`npm run audit:source-control` writes `.tmp/source-control-handoff/` JSON/Markdown for the GitHub handoff state. It records package/README repository references, local Git metadata, configured remotes, the GitHub remote refs/default branch with `git ls-remote`, persisted blocked/warning check counts, and a manual publication next-action checklist for missing `.git`, origin, refs, and re-audit steps; it is read-only and never initializes Git, commits, pushes, changes GitHub settings, validates hardware, or enables command upload.

`npm run audit:todo` writes `.tmp/todo-audit/` JSON/Markdown after `audit:completion`. It checks that unchecked items in `docs/SEEKR_GCS_ALPHA_TODO.md` and `docs/SEEKR_COMPLETION_PLAN.md` still cover the current real-world blocker categories, records top-level blocker/category summary counts, and fails if the completion audit artifact is missing or stale.
...
`npm run doctor` writes `.tmp/plug-and-play-doctor/` JSON/Markdown preflight evidence before final plug-and-play packaging or as a standalone local startup check. It checks package scripts including `setup:local`, package `engines`/`packageManager`, Node/package-lock readiness, `.npmrc` `engine-strict`, `.gitignore` protection for local secrets/generated artifacts including runtime `data/`, the latest source-control handoff artifact for `https://github.com/Ayush1298567/SEEKR`, local `tsx`/`concurrently`/`vite` dependency binaries needed by `npm run dev`, the `rehearsal:start` wrapper contract, project-local env setup, local Ollama model availability, API/client port availability, local data directory safety, and command-upload/hardware-actuation environment flags without validating hardware. Missing Git metadata or missing GitHub refs are recorded as non-blocking handoff warnings, while malformed or unsafe source-control handoff artifacts fail the local startup preflight.

`npm run rehearsal:start` is the plug-and-play local operator start command. It uses `.tmp/rehearsal-data` by default, declares expected read-only MAVLink/ROS/LiDAR/Isaac source categories for source-health visibility, runs non-destructive `npm run setup:local`, refreshes `npm run audit:source-control`, runs `npm run doctor`, then launches `npm run dev`; it does not validate hardware or enable command upload.

`npm run smoke:rehearsal:start` writes `.tmp/rehearsal-start-smoke/` JSON/Markdown by launching `npm run rehearsal:start` on temporary local API/client ports, checking `/api/config`, `/api/source-health`, `/api/readiness`, and the client shell, verifying command upload remains disabled, and shutting the process group down. It is a bounded local startup proof only; it does not validate hardware.

`npm run audit:plug-and-play` writes `.tmp/plug-and-play-readiness/` JSON/Markdown after acceptance, API probe, gstack QA/workflow evidence, source-control handoff evidence, TODO audit, setup, doctor, rehearsal-start smoke, and bundle verification. It proves the local app, local Ollama AI smoke, API readback, browser QA, review bundle, setup artifact, doctor artifact, rehearsal-start smoke artifact, operator quickstart, rehearsal start wrapper, and safety boundary are ready for an internal-alpha plug-and-play operator while keeping physical hardware blockers explicit. Source-control handoff gaps are reported as warnings only after the artifact passes the shared read-only semantic guard; unsafe or malformed source-control handoff artifacts fail. It fails if the doctor predates acceptance, omits any required package-script/runtime/repository-safety/source-control-handoff/start-wrapper/env/local-AI/port/data/safety row, has a non-passing critical doctor check, `docs/OPERATOR_QUICKSTART.md` omits setup/source-control audit/start, local AI, API evidence, source-health, real-world blockers, or disabled command/hardware authority, the operator start wrapper drops its local defaults or setup-before-source-control-before-doctor-before-dev ordering, the rehearsal-start smoke artifact is missing or failing, or the verified review bundle points at stale handoff, gstack workflow, gstack QA, TODO audit, source-control handoff, plug-and-play setup, plug-and-play doctor, rehearsal-start smoke, or packaged operator quickstart evidence.

`npm run handoff:bundle -- --label <review-name>` verifies the handoff digest table and requires passing or limitation-only gstack workflow-status with `gstackAvailable: true`, `pass-with-limitations` for limitation-only evidence, preserved manifest-level limitation details including unavailable gstack CLI details, preserved limitation details for stale or missing health/QA evidence, per-workflow skill availability, no-Git review limitations when the workspace lacks `.git`, health-history status/path, QA screenshot path preservation, TODO audit artifacts with all required blocker category IDs, matching top-level/category blocker counts, well-formed supporting TODO/blocker match evidence, a read-only source-control handoff artifact with local Git/GitHub publication limitations preserved as warnings, a passing plug-and-play setup artifact, a passing plug-and-play doctor artifact with repository-safety/source-control-handoff/runtime/startup evidence that is fresh against acceptance, a passing rehearsal-start smoke artifact, and operator quickstart coverage for setup/source-control audit/start, local AI, API evidence, source-health, real-world blockers, and disabled authority, then copies the handoff index, linked local artifacts, the workflow artifact, the workflow-referenced local gstack browser QA report and screenshots, the TODO audit artifact, the source-control handoff artifact, the plug-and-play setup artifact, the plug-and-play doctor artifact, the rehearsal-start smoke artifact, and `docs/OPERATOR_QUICKSTART.md` into `.tmp/handoff-bundles/` with a manifest for internal review. It is a packaging step only: all hardware claims and authorization fields remain false.

`npm run handoff:bundle:verify` verifies the latest copied bundle manifest, every copied file digest, copied gstack workflow semantics including perspective status/score/next-action detail, copied gstack QA report and screenshot inclusion plus report-content pass/command-safety, copied TODO audit blocker-category/top-level-count/support-shape semantics, copied source-control handoff semantics, copied plug-and-play setup env/data/safety semantics, copied plug-and-play doctor repository-safety/source-control-handoff/runtime/start-wrapper/AI/port/data/safety semantics and freshness against copied acceptance, copied rehearsal-start smoke startup/source-health/readiness/shutdown semantics, copied operator quickstart setup/source-control audit/start/AI/API/source-health/safety guidance, critical doctor-check pass status with only source-control/port/data-directory warnings allowed, and full high-confidence secret-scan coverage. Use `npm run handoff:bundle:verify -- --bundle .tmp/handoff-bundles/<file>.json` for a specific review bundle after transfer.

`npm run audit:goal` writes `.tmp/goal-audit/` JSON/Markdown after handoff verification, `audit:gstack`, `audit:todo`, bundling, bundle verification, and `audit:plug-and-play`. It restates the objective as a prompt-to-artifact checklist across named files, commands, safety boundary, acceptance/release/API probe evidence, completion audit, demo/handoff/bundle/gstack chain, plug-and-play readiness, TODO/blocker consistency, `docs/goal.md`, and explicit real-world blockers with a persisted remaining-blocker count/list.
