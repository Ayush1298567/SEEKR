...
`npm run audit:goal` writes `.tmp/goal-audit/` JSON/Markdown after handoff verification, `audit:gstack`, `audit:todo`, bundling, bundle verification, and `audit:plug-and-play`. It restates the objective as a prompt-to-artifact checklist across named files including `docs/OPERATOR_QUICKSTART.md`, commands, safety boundary, acceptance/release/API probe evidence, completion audit, demo/handoff/bundle/gstack chain, plug-and-play readiness with current source-control handoff and operator quickstart references, TODO/blocker consistency, `docs/goal.md`, and explicit real-world blockers with a persisted remaining-blocker count/list.
