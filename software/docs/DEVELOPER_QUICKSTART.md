...
`npm run demo:package -- --label <demo-name>` writes `.tmp/demo-readiness/` JSON/Markdown after acceptance and completion audit. It packages the latest acceptance, release checksum, command-boundary scan, API probe, completion audit, source-control handoff, hardware archive, policy gate, and overnight status pointers for an internal demo handoff while keeping all hardware claims false, listing real-world blockers from the audit, adding an operator/safety/DX/replay/demo-readiness perspective review, and adding a next-evidence checklist with the runbook and command that should produce each missing artifact. The DX perspective uses source-control handoff evidence when present, reports missing or unsafe GitHub publication state as a gap, and records clean published local HEAD evidence as ready for review. Stale overnight status is a warning, not a hardware validation claim.
