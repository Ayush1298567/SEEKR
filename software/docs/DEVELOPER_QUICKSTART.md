...
`npm run audit:source-control` writes `.tmp/source-control-handoff/` JSON/Markdown for the GitHub handoff state. It records package/README repository references, the parent GitHub landing README ordered fenced shell command blocks for a fresh-clone path into `SEEKR/software`, bounded smoke before strict local AI smoke before plug-and-play audit guidance, local Git metadata, configured remotes, the GitHub remote refs/default branch with `git ls-remote`, shallow fresh-clone contents for the landing README, `software/package.json`, `software/package-lock.json`, `software/.env.example`, `software/scripts/rehearsal-start.sh`, operator quickstart, shared operator-quickstart contract proof, and `npm ci --dry-run` install consistency, local HEAD versus the GitHub default branch SHA, clean-worktree status, an exact ordered source-control check list, persisted blocked/warning check counts, and a manual publication next-action checklist for missing or misordered GitHub landing guidance, incomplete fresh-clone contents, invalid published operator quickstart guidance, `.git`, origin, refs, unpublished HEAD, dirty worktrees, and re-audit steps. Shared validators reject a `github-landing-readme` pass unless it retains ordered fenced shell command proof through bounded smoke, strict local AI smoke, and plug-and-play audit plus the distinct `github-landing-readme-ai-readiness-proof` marker for final AI/readiness proof command guidance and reject a `fresh-clone-smoke` pass unless it retains the GitHub URL, exact shallow clone command, exact npm dry-run command, shared operator-quickstart contract proof, and every startup-critical `fresh-clone:` file evidence entry; the audit is read-only and never initializes Git, commits, pushes, changes GitHub settings, validates hardware, or enables command upload.
