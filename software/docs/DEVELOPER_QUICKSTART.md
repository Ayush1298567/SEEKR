...
`npm run ai:prepare` writes `.tmp/local-ai-prepare/` JSON/Markdown after running `ollama pull llama3.2` for the default local model. It is model setup evidence only and does not create command payloads, bypass validation, validate hardware, or enable command upload. Final plug-and-play readiness and review-bundle verification require the latest passing local AI prepare artifact, require its prepared model to match the latest acceptance strict local AI model, and reject copied prepare evidence unless the command is exactly three tokens, `ollama pull <model>` or an absolute `ollama` executable path plus `pull <model>`, with a non-option Ollama model token using only letters, numbers, dots, underscores, colons, slashes, or hyphens and artifact check evidence containing the exact command string. This keeps model setup from being skipped, swapped silently, or satisfied by a lookalike command.
