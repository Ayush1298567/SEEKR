...
`npm run ai:prepare` writes `.tmp/local-ai-prepare/` JSON/Markdown after running `ollama pull llama3.2` for the default local model. It is model setup evidence only and does not create command payloads, bypass validation, validate hardware, or enable command upload. Final plug-and-play readiness and review-bundle verification require the latest passing local AI prepare artifact, require its prepared model to match the latest acceptance strict local AI model, and reject copied prepare evidence whose command executable is not `ollama` or an absolute path ending in `ollama`, so model setup cannot be skipped, swapped silently, or satisfied by a lookalike command.
