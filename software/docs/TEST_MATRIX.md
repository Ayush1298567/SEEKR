...
- Recovery status: `npm run status:local` summarizes the latest acceptance, source-control handoff, fresh-clone strict AI proof, review-bundle verification, gstack health/QA, plug-and-play readiness, goal audit, overnight status, fallback-port warning, and remaining real-world blockers after an interrupted session without rerunning the full chain, and blocks recovery when acceptance points at stale release JSON/SHA/Markdown evidence, stale command-boundary JSON/Markdown evidence, stale command-boundary counts, strict local AI case names drift, or fresh-clone, plug-and-play, or review-bundle evidence is stale against the current source-control HEAD.
