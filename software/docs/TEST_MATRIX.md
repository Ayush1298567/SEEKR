...
- Review bundle summaries: top-level fresh-clone local HEAD, clone HEAD, source-control handoff local HEAD, source-control remote-default SHA, source-control shallow fresh-clone HEAD, source-control npm dry-run status, and source-control checked startup-file count, plus copied-bundle verification that those summaries match the copied source artifacts.
