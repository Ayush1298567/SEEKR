...
- Recovery status: `npm run status:local` summarizes the latest acceptance, API readback, source-control handoff, fresh-clone strict AI proof, review-bundle verification, gstack health/QA, plug-and-play readiness, goal audit, overnight status, fallback-port warning with listener diagnostics/details, and remaining real-world blockers after an interrupted session without rerunning the full chain, exposes local/remote/fresh-clone SHAs, strict local AI provider/model/URL/case count, review-bundle scan summary, and plug-and-play port diagnostics in terminal JSON, and blocks recovery when acceptance points at stale release JSON/SHA/Markdown evidence, stale command-boundary JSON/Markdown evidence, stale command-boundary counts, stale `/api/session` acceptance readback, strict local AI case names drift, downstream source-control/fresh-clone/review-bundle/gstack/plug-and-play/goal artifacts predate current acceptance, or fresh-clone, plug-and-play, or review-bundle evidence is stale against the current source-control HEAD.
