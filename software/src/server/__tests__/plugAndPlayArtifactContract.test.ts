...
describe("plug-and-play artifact contract", () => {
    ...
    const checks = doctorChecks().map((check) =>
      check.id === "local-ports"
        ? {
            ...check,
            status: "pass",
            details: "Default port(s) already in use on 127.0.0.1 by a non-SEEKR or unhealthy listener: api 8787. Listener diagnostics: api 8787 -> node pid 12345 cwd ~/Ayush/Prophet/prophet-console. npm run plug-and-play delegates to the rehearsal wrapper, which auto-selects free local API/client ports when no explicit port variables are set; stop the existing process only if you want SEEKR to use the default port(s). Current free fallback candidate(s): API 6100, client 5173; npm run plug-and-play prints the actual URLs it selects at startup.",
            evidence: [
              "PORT",
              "SEEKR_API_PORT",
              "SEEKR_CLIENT_PORT",
  ...
  it("rejects auto-recoverable pass evidence when fallback proof does not match the printed fallback ports", () => {
    const checks = doctorChecks().map((check) =>
      check.id === "local-ports"
        ? {
            ...check,
            ...
            evidence: [
              ...
              "fallback client port candidate 6201",
              "http://127.0.0.1:8787/api/health",
              "http://127.0.0.1:5173/",
              "lsof -nP -iTCP:8787 -sTCP:LISTEN",
              "listener 12345 cwd ~/Ayush/Prophet/prophet-console",
  ...
  it("rejects auto-recoverable pass evidence when a fallback candidate is still the occupied default port", () => {
    const checks = doctorChecks().map((check) =>
      check.id === "local-ports"
        ? {
            ...check,
            status: "pass",
            details: "Default port(s) already in use on 127.0.0.1 by a non-SEEKR or unhealthy listener: api 8787, client 5173. Listener diagnostics: api 8787 -> node pid 12345 cwd ~/Ayush/Prophet/prophet-console; client 5173 -> node pid 12346 cwd ~/Ayush/Prophet/prophet-console. npm run plug-and-play delegates to the rehearsal wrapper, which auto-selects free local API/client ports when no explicit port variables are set; stop the existing process only if you want SEEKR to use the default port(s). Current free fallback candidate(s): API 8787, client 6101; npm run plug-and-play prints the actual URLs it selects at startup.",
            evidence: [
              "PORT",
              "SEEKR_API_PORT",
              "SEEKR_CLIENT_PORT",
              "scripts/rehearsal-start.sh auto-selected free local API/client ports",
              "fallback API port candidate 8787",
              "fallback client port candidate 6101",
              "http://127.0.0.1:8787/api/health",
              "http://127.0.0.1:5173/",
              "lsof -nP -iTCP:8787 -sTCP:LISTEN",
